...
- **Route-slot category vs. Q2-preference suitability — two separate axes**: which place fills the Meal/Cafe/Discovery slot is decided strictly by the place's own category (`식사` / `카페·디저트` / `볼거리·문화·체험` or `산책·야간`); whether a place satisfies the user's explicit Q2 preference (food/walk/photo) is decided independently, from the place's own tag data. A place's category never implies a preference match, and a preference match never implies a route-slot role — a `카페·디저트` place, for example, can be a valid `photo`-preference match without ever being eligible to fill the Meal slot. **Preference-slot eligibility is a third, narrower concept layered on the tag axis for one option only**: the Preference *stop* of a `food`(먹방) route must additionally be a `식사` place, so 먹방 always yields a second real meal rather than a second cafe (every cafe record also carries the `food` tag) — `anything`/`walk`/`photo` are unconstrained beyond their tag match. See ADR-041. If no zone can produce a complete, distinct-place route honoring an explicit Q2 preference, that zone is not offered for that request rather than the engine silently substituting an unrelated place.
...
0. **INTRO** *(added `feat/product-visual-polish`, ADR-032)*: a soft entry gate — translucent veil + a ticket-style card (`★ ENTRY TICKET ★` / `대전 랜덤 여행, 시작해볼까요? ✨` / `여행 시작하기`) — layered over the same Setup card region Q1 uses, at an identical footprint on every breakpoint. The user must intentionally press the CTA to proceed; INTRO never auto-advances. **Restore exception (ADR-043):** when a tab already holds a drawn route (refresh, or a return from `/random-log`), that route is restored and the session resumes at RESULT — INTRO is intentionally not shown again, the Result Card comes back minimized, and BGM stays silent until the reader starts it, since only INTRO's CTA or the sidebar control begins playback.
...
- **Composer UX**: Opens from the Result Card flow, via the unified `“다시 뽑기”` CTA while the reroll reward is `locked`, **or** from the Memory Log write affordance on the landing page once a route exists (ADR-042/ADR-045 — a discoverability entry point only: it opens the same composer with the same attached route, and changes nothing about the reward lifecycle below). `/random-log` serves as the dedicated social proof archive and full log stream (read-only archive, no standalone composer there).
...
- **Composition**: **manual carousel** (ADR-038, supersedes ADR-028's original "one featured banner, no carousel" scope) — one banner visible at a time inside a fixed-aspect media viewport (so browsing never shifts the card's height, regardless of which item's raster loads), with prev/next chevrons, a position indicator, and touch swipe. Since ADR-042 it also advances on its own about every 10 seconds (wrapping last→first, restarted by any manual move) — but only while the card is genuinely on screen: at least half of it in the viewport, the tab visible, not hovered or focused, and never under reduced motion (ADR-047). An impression counts the same way, so a rail scrolled past the fold neither advances nor reports views. No tabs, no separate `자세히 보기` button — an item with a destination makes the banner itself the link, opened in a new tab.
...
| **TODAY'S DAEJEON Editorial Carousel** | **Fixed Content Decision** | Static TS data; 5 production banners, carousel browsable manually and auto-advancing ~10s (ADR-038, ADR-042), real outbound hyperlinks. No detail page, no Q2 seeding, no Slot funnel (ADR-015, revised). |
