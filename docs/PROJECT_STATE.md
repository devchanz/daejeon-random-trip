...
## Snapshot
- **Last Updated**: 2026-09-04 (analytics/GTM/UTM closeout pass, branch `feat/analytics-tracking`; supersedes the discovery/retention closeout entry below for "current branch" purposes)
- **Current `main` HEAD**: `1ac7476` (`feat: complete discovery and retention experience`) — the discovery/retention pass described in the TL;DR/Section A below is now committed and on `main`.
- **Current branch**: `feat/analytics-tracking`, branched from `main` @ `1ac7476` — GTM/GA4 custom-event/UTM-attribution implementation, **GTM Preview verified, not yet committed, not yet merged, not yet deployed**. Every change described in the new analytics bullet at the top of **A** exists only in this branch's working tree. See **D** for the full analytics status.
...
**IMPLEMENTED LOCALLY / GTM PREVIEW VERIFIED / NOT YET MERGED OR PRODUCTION-VERIFIED** (`feat/analytics-tracking`, ADR-039):
- **GTM-first architecture, direct GA4 fallback**: `src/app/layout.tsx` renders `GoogleTagManager` when `NEXT_PUBLIC_GTM_ID` is set, else falls back to the pre-existing direct `GoogleAnalytics` loader when only `NEXT_PUBLIC_GA_MEASUREMENT_ID` is set, else neither — the two can never render together, so `page_view` has exactly one source regardless of which env var is configured.
- **16 approved custom events implemented**: `landing_view`, `intro_start`, `q1_select`, `q2_select`, `spin`, `result_view`, `route_cta`, `place_map_click`, `share`, `reroll`, `todays_daejeon_view`, `todays_daejeon_next`, `todays_daejeon_prev`, `todays_daejeon_click`, `explore_more_click`, `random_log_submit` — all dispatched through one central whitelisting/sanitizing module (`src/lib/analytics/`), never `window.dataLayer` directly from a component. Zero PII/free-text (never nickname, Memory Log message, place names, `share_code`).
- **First-touch session UTM attribution**: `src/lib/attribution/utmSession.ts`, `sessionStorage`-only, `utm_term` never captured, auto-attached to every event.
...
- **GTM container `GTM-K9G6TQBM` exists and is configured in Preview** — a single generic `GA4 Event - Product Events` tag (Event Name = `{{Event}}`) reads all 16 events via one `CE - Product Analytics Events` Custom Event trigger, rather than 16 per-event tags. **Not published.**
- **A real stale-Data-Layer-Variable leak was found in GTM Preview and fixed**: an earlier event's `editorial_position` was still resolving during a later, unrelated `q1_select`, because GTM's dataLayer resolves against a running merged model rather than a per-push snapshot. Fixed by having every push explicitly reset all 9 event-specific parameter keys to `undefined` before applying the current event's own values; re-verified fixed in Preview.
- **Not yet true**: this branch is not committed/merged/deployed; the GTM container is not published; production Tag Assistant/GA4 DebugView verification against `https://daejeon-random-trip.vercel.app/` has not been performed (blocked on merge + deploy + publish, in that order); the Campaign & Funnel Analysis Dashboard (Looker Studio or similar, `docs/ANALYTICS.md` §8) is not built; shared-route recipient interaction events (`shared_route_view`, `shared_route_guide_open`, `shared_route_slot_click`) remain deferred/not implemented.
- **Privacy contract (unchanged, now enforced in code rather than spec-only)**: never transmit Memory Log nicknames, Memory Log free text, `share_code`, IP, email, phone, or other PII. `place_map_click` is the primary proxy conversion and must never be represented as proof of a physical visit.
...
- ~~Custom GA4 event instrumentation~~ — **implemented**, `feat/analytics-tracking`, ADR-039 (see **D**); GTM Preview verified, not yet merged/deployed. **Campaign/Funnel Dashboard (Looker Studio) remains deferred** — spec exists (`docs/ANALYTICS.md` §8), zero code/dashboard exists yet.
...
- ~~GA4 / GTM custom instrumentation~~ — **implemented and GTM Preview verified** (container `GTM-K9G6TQBM`, not published), `feat/analytics-tracking`, ADR-039 — see **D**.
...
- ~~Editorial Rail (TODAY'S DAEJEON) banner-click/impression tracking~~ — **done**: hrefs (ADR-038, all 5 items) and `todays_daejeon_view/next/prev/click` impression/click events (ADR-039, `feat/analytics-tracking`, GTM Preview verified) are both implemented.
...
1. **Close out `feat/analytics-tracking`** (see **D**, ADR-039) — implementation complete, GTM Preview verified. Remaining steps, in order: (a) commit/merge the branch into `main`; (b) publish the `GTM-K9G6TQBM` container; (c) deploy to production (`NEXT_PUBLIC_GTM_ID` is already set in Vercel Production, so no further env change is needed); (d) run a production Tag Assistant/GA4 DebugView check against `https://daejeon-random-trip.vercel.app/`; (e) build the Campaign & Funnel Analysis Dashboard (Looker Studio or similar) once real collection has begun. Committing/pushing/PR/merge/publish/deploy all require explicit user instruction per **H**; none performed as part of this closeout.
2. **Merge/integrate `feat/todays-daejeon-discovery`** — complete, Human Browser E2E approved. Note: per the Snapshot above, this work is already reflected in `main` @ `1ac7476`; if that commit has not actually been pushed/merged through the normal branch lifecycle, reconcile this line with real repository state before treating it as closed.
...
6. **Documentation gap**: a dedicated ADR for the Result Card's own raster skin redesign from `6560a5b` (still current/unchanged, never separately recorded) remains unwritten. The GA4 base integration (`6ae03c4`) gap is now superseded by ADR-039, which covers the current GTM/analytics architecture built on top of it. Governance/documentation gap, not a code defect.
