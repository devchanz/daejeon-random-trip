...
export const metadata: Metadata = {
  title: '랜덤 로그 | 대전 랜덤 여행',
  description: '대전 랜덤 여행을 뽑아본 사람들이 남긴 랜덤 로그를 확인해보세요.',
  ...
  return (
    <div className="flex min-h-screen flex-col bg-retro-dots text-[#2b2520]">
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-6 sm:px-6 sm:py-10">
        <Link
          href="/"
          className="mb-4 inline-flex w-fit items-center gap-1.5 rounded-full border-2 border-[#2b2520] bg-[#fffef9] px-3.5 py-1.5 text-xs font-black text-[#2b2520] shadow-2xs transition-colors hover:bg-[#faf6ee] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#ff5555] focus-visible:outline-offset-1"
        >
        ...
        <div className="mb-6 flex flex-col gap-1.5">
          ...
          <h1 className="text-xl font-black text-[#2b2520] sm:text-2xl">랜덤 로그</h1>
          <p className="text-xs font-bold text-[#6b6257] sm:text-sm">
            대전 여행을 뽑아본 사람들이 남긴 랜덤 로그예요.
          </p>
        </div>
        ...
        {!result.success ? (
          <div
            ...
            className="flex flex-col items-center gap-2 rounded-2xl border-2 border-dashed border-[#d8d0c2] bg-[#faf6ee] p-10 text-center"
          >
            <span className="text-3xl" role="img" aria-label="Error">
              ⚠️
            </span>
            <p className="text-sm font-bold text-[#6b6257]">
              랜덤 로그를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.
            </p>
          </div>
        ) : (
          <RandomLogList
